...
- Executable `maquila run`: single-host lock, restart cleanup, pinned Node/Bun bootstrap, remote planner/worker/documenter/verification/reviewer, evidence and patch harvest, VM destruction, then controller-side bot branch and ready-for-review pull-request publication. Planner-classified web UI work installs pinned `agent-browser`/Chromium in VM, requires 2–10 validated screenshots, attempts video/contact-sheet capture, includes visuals in fresh review, and attaches them from host to PR body before a draft becomes ready. A structured `model_request_failed` result during the post-plan lifecycle gets up to three controller-owned attempts under the same run ID; each retry preserves prior evidence and rebuilds a fresh VM at the pinned base SHA.
...
- Maquila-owned `intake deploy/status/destroy` lifecycle for one trusted exe.dev controller host: exact-build upload, pinned runtime bootstrap, dedicated registered exe.dev key, target checkout, owner-only credential transfer, user service, HTTPS proxy, marked Linear webhook, verified intake, page-durable bounded assigned-`Todo`/`maquila-ready` reconciliation, persistent issue-to-run claims, controller-time eligibility admission before VM creation, idempotent Linear dashboard-link delivery, optional controller-owned TTL teardown, and rollback. Omitted TTL remains indefinite. `intake serve` and `intake expire` remain internal host processes. Manual starts remain unchanged.
...
- `maquila setup` and `maquila doctor` for strict XDG config, hidden TTY paste of Linear and OpenRouter keys, environment credentials, optional `op://` references, optional user-scope Pi skill, credential checks, and remediation. Opt-in `doctor --issue ID --require-label LABEL` validates assigned Todo intake and an exact label without starting work. Managed skill copies are repeatable when byte-identical; legacy managed links remain supported. This preflight is not a reservation or automatic trigger gate.
...
- Controller-side credential precedence for GitHub (`GITHUB_TOKEN`, `GH_TOKEN`, `gh auth token`) and Linear (`LINEAR_API_TOKEN`, saved host key, then `op read`); OpenRouter (`OPENROUTER_API_KEY`, saved host key, then `op read`); optional exe.dev identity with OpenSSH config/agent support and no agent forwarding.
...
- Credentialed persistent-controller-VM acceptance proof across a real labeled Linear issue. The deployed user service has restart-on-failure but has not proven restart-after-host-reboot behavior.
...
Controller evidence lives at `.maquila/controllers/<run-id>/`. Current workflow artifacts include `workflow-manifest.json` version 2, harvested `workflow-execution.json` version 1, `controller-state.json` version 2, `change.patch`, `evidence-manifest.json`, optional validated `ui-evidence.json` plus media, and `publication.json` after publication. Host telemetry lives at `.maquila/telemetry/<run-id>.jsonl`. Batch coordination metadata lives at `.maquila/batches/<batch-id>/batch.json`; per-run evidence remains authoritative. Hash links prove consistency, not authenticity against a compromised VM.
