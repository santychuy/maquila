...
For `exe/claude-sonnet-4-6` only, controller records explicit reference mapping to `anthropic/claude-sonnet-4-6` and pinned models.dev commit, path, and source hash. Reference estimate uses reviewed flat Anthropic list rates and exact integer nano-USD math: `input × 3000 + output × 15000 + cacheRead × 300 + cacheWrite × 3750`. Missing/incompatible rates, unsafe values, tiers, or extra price dimensions leave estimate unavailable. Runtime limits remain 200,000 context and 16,384 output; displayed Anthropic reference limits of 1,000,000 and 128,000 do not alter execution. Injected SDK rates remain zero, so receipt cost is not billing evidence. Catalog refresh is manual and reviewed; deliberate staleness stays visible through provenance and dates. Old runs are neither backfilled nor repriced. Estimates never affect budgets, gates, workflow state, or publication.

Run detail presents horizontally scrollable actor/phase buttons from host `recordedAt` phase boundaries. Open segments grow each poll; completed segments freeze at host finish time. Keyboard or pointer selection shows actor, status, timing, safe role/model metadata, tools, reported tokens, prompt fingerprint, and non-billable reference-estimate state. Raw events remain a collapsed drill-down. Polling preserves focused and selected segments and drains bounded cursor pages without overlapping ticks. Old runs remain usable with unavailable metadata fields.
