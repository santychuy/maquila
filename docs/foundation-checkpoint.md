...
The agreed live pilot is now `santychuycom/santychuy.com`, local first, with one approved issue and the exact `maquila-ready` label authorizing future automatic selection. Isolated npm tarball installation and onboarding precede a manual Linear-to-PR test; automatic triggering needs a separately approved second issue. The selected first change is a bilingual keyboard-accessible skip link. A dedicated $10 total OpenRouter key allowance must be verified before paid execution; exe.dev charges remain separate. No watcher or durable queue is enabled. The manual controller does not enforce this label, and preflight is not a reservation. Cross-platform package validation, registry publication, and real-Linear-to-real-PR acceptance are separate pending gates before automatic intake or always-on hosting.
