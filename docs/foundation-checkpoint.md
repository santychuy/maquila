...
`doctor --target PATH --issue ID [--require-label LABEL] --json` adds a read-only assigned-Todo Linear eligibility check and optional exact label match. Invalid options fail before vendor calls; transport errors are not exposed in output. Generic setup/doctor do not probe a Linear issue. Managed skill installation accepts identical copies and existing managed links, but does not overwrite differing or unrelated content.
...
GitHub visibility for `santychuy/maquila` was changed to public and verified. The new release changes are still local until committed and pushed. npm authentication returned `401 Unauthorized`; registry publication, registry-install verification, and a final real-source release tarball remain pending. Do not begin the Bookbounce pilot based only on this local checkpoint.
