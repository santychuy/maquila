...
`maquila intake deploy --target PATH` creates and owns the single-repository controller lifecycle: exact local package upload, pinned runtime bootstrap, dedicated exe.dev key registration, target clone, private credential transfer, user service, public HTTPS proxy, and marked Linear webhook. Optional `--ttl 24h` installs a persistent controller-side timer that invokes full teardown; omission remains indefinite. `intake status` reports the deadline; `intake destroy` revokes webhook/key/proxy and deletes the VM. `maquila intake serve --target PATH --port PORT` and `intake expire` remain internal host processes. Serve requires `MAQUILA_LINEAR_WEBHOOK_SECRET` and an HTTPS origin in `MAQUILA_PUBLIC_URL`. That process:
