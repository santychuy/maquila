...
```text
Linear webhook/reconciliation -> assigned Todo + maquila-ready admission
  -> immutable intake -> fresh VM at pinned SHA -> plan
...
Remote controller composition, local observation, GitHub publication, browser-backed UI evidence, and OpenRouter-backed role execution are implemented. Planner-classified web UI runs install pinned `agent-browser`/Chromium in VM, require validated screenshots, attempt video/contact-sheet capture, and attach host-harvested media while PR remains draft; GitHub write authority stays host-side. Each agent definition owns its pinned `openrouter/<provider>/<model>` identifier; current defaults are `openrouter/google/gemini-3.7-flash` for worker/documenter/reviewer and `openrouter/z-ai/glm-5.3` for planner. Runtime does not ship a complete model catalog; doctor and remote bootstrap validate pinned identifiers against OpenRouter's live catalog. VM is not a security sandbox. `maquila run` snapshots a Linear `Todo` issue and GitHub base SHA, holds a single-host lock, reconciles abandoned VMs, bootstraps pinned runtimes in fresh exe.dev VMs, streams safe telemetry through deterministic phase boundaries, runs planner, sequential worker/documenter, verification, and fresh-reviewer sessions, binds harvested evidence to the reviewed patch, and destroys the final VM. A structured post-plan `model_request_failed` result triggers bounded controller recovery in a replacement VM while retaining the same run ID, pinned inputs, manifest, attempt telemetry, and prior-attempt evidence. The trusted controller then checks the pinned base, applies the patch in a temporary clone, creates a deterministic bot branch and commit, opens a ready-for-review pull request, and records publication metadata before completion.

`maquila run start` returns accepted run identity before completion. Persistent on-demand observer binds loopback, replays canonical host JSONL, serves read-only run list/detail/event views, and links the completed pull request. `maquila intake deploy/status/destroy` owns one persistent exe.dev controller lifecycle: exact-build upload, pinned runtime, dedicated exe.dev key, target checkout, explicit credential transfer, persistent user service, HTTPS proxy, team-scoped Linear webhook, and rollback. Its internal `intake serve` process provides verified deadline-bounded webhook intake, page-durable reconciliation, persistent issue-to-run claims, controller-time `maquila-ready` admission before launch acknowledgement, idempotent Linear dashboard-link delivery, and a separate run-token HTTPS gateway. Observer remains loopback-only; gateway access is read-only and expires after 24 hours. Manual starts keep their current intake contract. Maquila-owned Pi skill routes lifecycle/start/status requests through these commands. Observer, gateway, intake service, and skill hold no acceptance authority.
