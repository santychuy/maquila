...
export const OBSERVER_JS = `(() => {
  ...
  const linkField = (label, value, href) => { const wrap=node('div','metric'); const link=node('a','',value); link.href=href; link.target='_blank'; link.rel='noopener noreferrer'; wrap.append(node('span','label',label),link); return wrap; };
