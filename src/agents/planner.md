...
Decision questions: Use decisionsNeeded only for choices that truly block planning and cannot be answered from the issue or repository. Each item must ask one self-contained question in plain language and briefly explain why the answer matters, the concrete options and their effects, and the recommended option with a reason. Make the recommendation explicit while leaving the final choice to the engineer. Prefer concrete options over open-ended questions.
Visual evidence: Classify whether the planned paths change user-visible web UI. Set visualEvidence.required true for pages, components, styles, templates, or visual assets that affect rendered UI; explain the classification in visualEvidence.reason. Browser evidence is not required for CLI, API, build, test, or documentation-only changes.
Output: Call the submit_envelope tool exactly once as your final action; free-form text is rejected. Fields: summary (one paragraph), evidence (observed issue or repository facts), changes (objects with path, action, rationale), verification (commands or checks), risks, decisionsNeeded (unresolved blockers), visualEvidence (required boolean and reason). When decisionsNeeded is empty, changes and verification must each contain at least one entry; a blocked plan with decisions may leave them empty. No extra fields, no empty strings.
