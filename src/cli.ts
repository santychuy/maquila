...
Lists validated specialist definitions or runs read-only planner.
`;

export function parseCli(args: string[]): PlanOptions | "help" | "list-agents" {
  ...
  if (positionals.join(" ") !== "pi plan") {
    throw new Error("Expected command: agents list or pi plan");
  }
